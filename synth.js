...
let fwFxReady = false;  // lazily-built firework reverb/echo bus
let fwDry, fwReverb, fwReverbGain, fwDelay, fwDelayGain, fwFeedback;
...
// A shared "distant" FX bus for the booms: a long reverb tail plus a feedback
// echo, so blasts sound like they're rolling in from across the water.
function initFwFx() {
  if (fwFxReady || !ac || !comp) return;
  fwDry = ac.createGain(); fwDry.gain.value = 0.75; fwDry.connect(comp);
  fwReverb = ac.createConvolver(); fwReverb.buffer = makeImpulse(3.2, 3);
  fwReverbGain = ac.createGain(); fwReverbGain.gain.value = 1.1;
  fwReverb.connect(fwReverbGain); fwReverbGain.connect(comp);
  fwDelay = ac.createDelay(1.0); fwDelay.delayTime.value = 0.33;
  fwFeedback = ac.createGain(); fwFeedback.gain.value = 0.45;
  fwDelayGain = ac.createGain(); fwDelayGain.gain.value = 0.6;
  fwDelay.connect(fwFeedback); fwFeedback.connect(fwDelay);
  fwDelay.connect(fwDelayGain); fwDelayGain.connect(comp);
  fwFxReady = true;
}

// A synthesized firework "boom": a lowpass-swept noise burst plus a low thump,
// panned to where the burst appeared, scaled by `level` (bigger = louder), and
// sent through the distant reverb/echo bus.
function fireworkBoom(pan, level) {
  if (!ac || !comp) return;
  initFwFx();
  level = level || 1;
  const t = ac.currentTime;
  ...
  out.connect(fwDry); out.connect(fwReverb); out.connect(fwDelay);

  // Boom: filtered noise sweeping down. Bigger blasts are lower and longer.
  const dur = (0.35 + Math.random() * 0.3) * (0.8 + level * 0.4);
  const src = ac.createBufferSource();
  src.buffer = fwNoiseBuf; src.loop = true;
  const lp = ac.createBiquadFilter();
  lp.type = 'lowpass';
  lp.frequency.setValueAtTime((700 + Math.random() * 700) / (0.6 + level * 0.4), t);
  lp.frequency.exponentialRampToValueAtTime(90, t + dur);
  const g = ac.createGain();
  g.gain.setValueAtTime(0.0001, t);
  g.gain.exponentialRampToValueAtTime(Math.min(0.6, 0.26 * level), t + 0.012);
  ...
  const o = ac.createOscillator();
  o.type = 'sine';
  o.frequency.setValueAtTime(150 / (0.7 + level * 0.3), t);
  o.frequency.exponentialRampToValueAtTime(40, t + 0.18 * level);
  const og = ac.createGain();
  og.gain.setValueAtTime(0.0001, t);
  og.gain.exponentialRampToValueAtTime(Math.min(0.5, 0.24 * level), t + 0.01);
  og.gain.exponentialRampToValueAtTime(0.0001, t + 0.26 * level);
  o.connect(og); og.connect(out);
  o.start(t); o.stop(t + 0.3 * level + 0.1);
}

// ============================================================
// FOURTH OF JULY — a lakeside fireworks show
// ============================================================
const TAU = Math.PI * 2;
const FW_PALETTES = [
  ['#ff3b3b', '#ffffff', '#4d7bff'], // red / white / blue
  ['#ffd23f', '#ff7ac2', '#ffffff'], // gold / pink / white
  ['#3fffd2', '#4d7bff', '#c86bff'], // teal / blue / violet
  ['#ff7a3f', '#ffd23f', '#ff3b3b'], // warm embers
  ['#8affc1', '#ffffff', '#4dff88'], // greens
];
// Burst "recipes" — size, particle count, spread, gravity, loudness, look.
const FW_BURSTS = [
  { name: 'small',  weight: 3, count: [24, 38],  speed: 2.2, decay: [0.014, 0.024], grav: 0.03,  boom: 0.7, flash: 0.25, rad: 90  },
  { name: 'medium', weight: 4, count: [55, 85],  speed: 3.4, decay: [0.008, 0.015], grav: 0.03,  boom: 1.0, flash: 0.4,  rad: 150 },
  { name: 'big',    weight: 2, count: [120, 170],speed: 5.4, decay: [0.005, 0.009], grav: 0.028, boom: 1.7, flash: 0.85, rad: 240 },
  { name: 'willow', weight: 2, count: [70, 100], speed: 2.8, decay: [0.004, 0.007], grav: 0.075, boom: 1.0, flash: 0.4,  rad: 150, gold: true },
  { name: 'ring',   weight: 2, count: [56, 72],  speed: 3.8, decay: [0.008, 0.012], grav: 0.02,  boom: 1.0, flash: 0.4,  rad: 150, ring: true },
  { name: 'crackle',weight: 2, count: [50, 80],  speed: 3.2, decay: [0.009, 0.016], grav: 0.03,  boom: 1.1, flash: 0.45, rad: 150, crackle: true },
];
...
function buildFireworks() {
  ...
  let raf = null, W = 0, H = 0, dpr = 1, waterY = 0, tick = 0;
  let rockets = [], sparks = [], flashes = [], boats = [], stars = [], sinceLaunch = 0;
  const GRAV = 0.06;
  ...
  function makeStars() {
    stars = [];
    const n = Math.round((W * waterY) / 11000);
    for (let i = 0; i < n; i++) {
      stars.push({ x: Math.random() * W, y: Math.random() * waterY * 0.85,
                   r: rnd(0.4, 1.3), p: Math.random() * TAU });
    }
  }
  function makeBoats() {
    boats = [];
    ...
    for (let i = 0; i < n; i++) {
      const dir = i % 2 ? 1 : -1;
      boats.push({
        x: Math.random() * W, dir, speed: rnd(0.12, 0.28),
        scale: rnd(0.8, 1.5), light: ['#ffbe5a', '#ff8a5a', '#fff2c0'][(Math.random() * 3) | 0],
      });
    }
  }

  function pickBurst() {
    let total = 0; for (const b of FW_BURSTS) total += b.weight;
    let r = Math.random() * total;
    for (const b of FW_BURSTS) { if ((r -= b.weight) <= 0) return b; }
    return FW_BURSTS[1];
  }

  function launch() {
    const x = W * (0.1 + Math.random() * 0.8);
    const targetY = waterY * (0.12 + Math.random() * 0.5);
    const pal = FW_PALETTES[(Math.random() * FW_PALETTES.length) | 0];
    rockets.push({
      x, y: waterY, targetY,
      vx: (Math.random() - 0.5) * 1.4,
      vy: -Math.sqrt(2 * GRAV * (waterY - targetY)),
      color: pal[0], pal, burst: pickBurst(),
    });
  }

  function explode(x, y, pal, b) {
    const level = b.boom * (0.9 + Math.random() * 0.2);
    fireworkBoom((x / W) * 2 - 1, level);
    // Atmospheric light: a soft flash that lights up the sky/haze.
    flashes.push({ x, y, color: pal[(Math.random() * pal.length) | 0],
                   r: b.rad, life: 1, decay: b.name === 'big' ? 0.03 : 0.05, power: b.flash });
    const n = (rnd(b.count[0], b.count[1])) | 0;
    for (let i = 0; i < n; i++) {
      let a, sp;
      if (b.ring) { a = (i / n) * TAU; sp = b.speed * rnd(0.92, 1.08); }
      else { a = Math.random() * TAU; sp = Math.random() * b.speed + 0.4; }
      let color;
      if (b.gold) color = ['#ffd23f', '#ffae3f', '#fff0b0'][(Math.random() * 3) | 0];
      else if (b.ring) color = pal[0];
      else color = Math.random() < 0.18 ? '#ffffff' : pal[(Math.random() * pal.length) | 0];
      sparks.push({
        x, y, vx: Math.cos(a) * sp, vy: Math.sin(a) * sp, color,
        life: 1, decay: rnd(b.decay[0], b.decay[1]), grav: b.grav,
        twinkle: Math.random() < 0.35,
        crackle: b.crackle && Math.random() < 0.5,
        size: b.name === 'big' ? 2.6 : 2,
      });
    }
  }
  ...
  function drawStars() {
    ...
    for (const s of stars) {
      const a = 0.25 + 0.25 * Math.sin(tick * 0.05 + s.p);
      ctx.globalAlpha = a;
  ...
  function drawFlashes() {
    ...
    for (let i = flashes.length - 1; i >= 0; i--) {
      ...
      const a = f.life * f.power;
      g.addColorStop(0, hexA(f.color, a));
  ...
  function drawBoat(b) {
    const s = b.scale, hw = 22 * s, hh = 7 * s;
    ctx.save();
    ctx.translate(b.x, waterY);
    ctx.globalCompositeOperation = 'source-over';
    ctx.fillStyle = '#0a0e1a';
    ctx.beginPath();
    ctx.moveTo(-hw, -hh); ctx.lineTo(hw, -hh);
    ctx.lineTo(hw - 4 * s, 0); ctx.lineTo(-hw + 4 * s, 0);
    ctx.closePath(); ctx.fill();
    ctx.fillRect(-5 * s, -hh - 6 * s, 10 * s, 6 * s);           // cabin
    ctx.strokeStyle = '#141a2c'; ctx.lineWidth = 1.1 * s;
    ctx.beginPath(); ctx.moveTo(0, -hh - 6 * s); ctx.lineTo(0, -hh - 18 * s); ctx.stroke();
    ctx.globalCompositeOperation = 'lighter';                    // warm lamp
    ctx.fillStyle = hexA(b.light, 0.95);
    ctx.beginPath(); ctx.arc(hw - 6 * s, -hh - 2 * s, 1.7 * s, 0, TAU); ctx.fill();
    ctx.restore();
    ctx.globalCompositeOperation = 'source-over';
  }
  ...
  function drawWater() {
    ...
    wg.addColorStop(0, 'rgba(12,17,38,0.82)');
    wg.addColorStop(1, 'rgba(3,5,14,0.96)');
    ctx.fillStyle = wg; ctx.fillRect(0, waterY, W, H - waterY);
    // Rippled reflection of the sky, dimming with depth.
    ctx.globalCompositeOperation = 'lighter';
    const wh = H - waterY, strip = 6;
    for (let dy = 0; dy < wh; dy += strip) {
      const srcY = waterY - dy - strip;
      if (srcY < 0) break;
      const wob = Math.sin(dy * 0.06 + tick * 0.06) * (1.5 + dy * 0.04);
      ctx.globalAlpha = Math.max(0, 0.34 * (1 - dy / wh));
      ctx.drawImage(canvas, 0, srcY * dpr, W * dpr, strip * dpr,
                    wob, waterY + dy, W, strip);
    }
    ctx.globalAlpha = 1;
    ctx.globalCompositeOperation = 'source-over';
    // Faint horizon glow.
    ctx.fillStyle = 'rgba(120,140,205,0.16)';
    ctx.fillRect(0, waterY - 1, W, 2);
  }

  function frame() {
    tick++;
    // Fade only the sky (water is redrawn each frame).
    ctx.globalCompositeOperation = 'source-over';
    ...
    for (let i = rockets.length - 1; i >= 0; i--) {
      ...
      ctx.beginPath(); ctx.arc(r.x, r.y, 2.2, 0, TAU); ctx.fill();
      if (r.vy >= 0 || r.y <= r.targetY) { explode(r.x, r.y, r.pal, r.burst); rockets.splice(i, 1); }
    }
    for (let i = sparks.length - 1; i >= 0; i--) {
      const s = sparks[i];
      s.x += s.vx; s.y += s.vy; s.vy += s.grav; s.vx *= 0.985; s.vy *= 0.985;
      s.life -= s.decay;
      if (s.life <= 0 || s.y >= waterY) { sparks.splice(i, 1); continue; }
      let a = Math.max(0, s.life);
      if (s.twinkle && s.life < 0.5) a *= 0.4 + 0.6 * Math.abs(Math.sin(tick * 0.5 + s.x));
      ctx.globalAlpha = a;
      ctx.fillStyle = s.color;
      ctx.beginPath(); ctx.arc(s.x, s.y, s.size, 0, TAU); ctx.fill();
      if (s.crackle && Math.random() < 0.08) {   // secondary crackle sparks
        ctx.globalAlpha = a * 0.8;
        ctx.beginPath(); ctx.arc(s.x + rnd(-4, 4), s.y + rnd(-4, 4), 1, 0, TAU); ctx.fill();
      }
    }
    ctx.globalAlpha = 1;

    // Boats drift along the waterline (drawn into the sky region so the water
    // reflection picks up their lights).
  ...
  function open() {
    ...
    resize();
    rockets = []; sparks = []; flashes = []; sinceLaunch = 100;
